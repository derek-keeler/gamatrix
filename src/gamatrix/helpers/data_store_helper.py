...
class DataStoreHelper:
    ...
    def load_data_store(self) -> Optional[DataStore]:
        ...
        if not os.path.exists(self.data_store_path):
            self.log.info(f"Data store file {self.data_store_path} does not exist")
            return None
    ...
    def save_data_store(self, data_store: DataStore) -> bool:
        ...
        try:
            ...
            data_dict = {
                'users': {str(k): asdict(v) for k, v in data_store.users.items()},
            ...
            self.log.info(f"Data store saved successfully to {self.data_store_path}")
            return True
            ...
            for i in range(self.max_backups - 1, 0, -1):
                ...
                if os.path.exists(old_backup):
                    if i == self.max_backups - 1:
                        ...
                        self.log.debug(f"Rotated backup: {old_backup} -> {new_backup}")
    ...
    def update_user_data(self, data_store: DataStore, user_id: int, 
                        user_data: UserData, games: Dict[str, GameData]) -> DataStore:
        """
        ...
        for release_key, game_data in games.items():
            if release_key in data_store.games:
                ...
                if user_id in game_data.installed and user_id not in existing_game.installed:
                    existing_game.installed.append(user_id)
                elif user_id not in game_data.installed and user_id in existing_game.installed:
                ...
                if existing_game.max_players is None and game_data.max_players is not None:
    ...
    def remove_user_data(self, data_store: DataStore, user_id: int) -> DataStore:
        """
