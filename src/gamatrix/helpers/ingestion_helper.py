...
class IngestionHelper:
    ...
    def extract_user_data(
        self, user_id: int, db_path: str
    ) -> Tuple[UserData, Dict[str, GameData]]:
        """
        ...
        try:
            with open(db_path, "rb") as f:
                ...
                if not header.startswith(b"SQLite format 3"):
                    raise ValueError(
                        f"File is not a valid SQLite database: {db_path}"
                    )  # noqa: E501
        ...
        games_data = {}
        total_games = len(all_games)  # noqa: F841
        installed_count = 0
        ...
        for release_key, game_info in all_games.items():
            try:
                ...
                if "title" not in game_info or not game_info["title"]:
                    self.log.warning(
                        f"Game {release_key} has no title, skipping"
                    )  # noqa: E501
                    continue

                if "platforms" not in game_info or not game_info["platforms"]:
                    self.log.warning(
                        f"Game {release_key} has no platforms, using 'unknown'"
                    )
                ...
                game_data = GameData(
                    ...
                    slug=game_info.get(
                        "slug", release_key.lower().replace(" ", "-")
                    ),  # noqa: E501
                ...
                self.log.error(
                    f"Error processing game {release_key} for user {user_id}: {e}"  # noqa: E501
                )
                continue
        ...
        try:
            user_data = UserData(
                user_id=user_id,
                username=self.config["users"][user_id].get(
                    "username", f"User_{user_id}"
                ),
                db_filename=self.config["users"][user_id].get(
                    "db", "unknown.db"
                ),  # noqa: E501
                db_mtime=time.strftime(
                    constants.TIME_FORMAT,
                    time.localtime(os.path.getmtime(db_path)),  # noqa: E501
                ),
                total_games=len(
                    games_data
                ),  # Use actual processed games count  # noqa: E501
                installed_games=installed_count,
            )
        except Exception as e:
            raise Exception(
                f"Error creating user data for user {user_id}: {e}"
            )  # noqa: E501

        self.log.info(
            f"Successfully extracted {len(games_data)} games for user {user_id} "  # noqa: E501
    ...
    def process_all_users(self) -> Dict[str, GameData]:
        ...
        all_games: Dict[str, GameData] = {}

        for user_id in self.config["users"]:
            if "db" not in self.config["users"][user_id]:
                self.log.warning(
                    f"User {user_id} has no database file configured"
                )  # noqa: E501
                continue

            db_path = f"{self.config['db_path']}/{self.config['users'][user_id]['db']}"  # noqa: E501

            if not os.path.exists(db_path):
                self.log.warning(
                    f"Database file not found for user {user_id}: {db_path}"
                )
                continue

            try:
                user_data, user_games = self.extract_user_data(
                    user_id, db_path
                )  # noqa: E501
                ...
                for release_key, game_data in user_games.items():
                    if release_key in all_games:
                        ...
                        if (
                            not existing_game.multiplayer
                            and game_data.multiplayer  # noqa: E501
                        ):  # noqa: E501
                        ...
                        if (
                            ...
                            and game_data.max_players
                            > existing_game.max_players  # noqa: E501
                        ):
                            existing_game.max_players = game_data.max_players
                    else:
                        # Add new game
        ...
        self.log.info(
            f"Processed all users, found {len(all_games)} unique games"
        )  # noqa: E501
        return all_games
