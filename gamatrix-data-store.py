...
def rebuild_data_store(config, force=False):
    ...
    for user_id in config['users']:
        ...
        if not os.path.exists(db_path):
            print(f"Warning: Database file not found for user {user_id}: {db_path}, skipping")
            continue
        
        try:
            print(f"Processing user {user_id} ({config['users'][user_id]['username']})...")
            user_data, user_games = ingestion_helper.extract_user_data(user_id, db_path)
            data_store = data_store_helper.update_user_data(data_store, user_id, user_data, user_games)
            users_processed += 1
    ...
    if data_store_helper.save_data_store(data_store):
        print(f"Successfully rebuilt data store with {len(data_store.users)} users and {len(data_store.games)} games")
        return True
    else:
    ...
    for release_key, game_data in data_store.games.items():
        ...
        for owner_id in game_data.owners:
            if owner_id not in data_store.users:
                print(f"Error: Game {release_key} owned by non-existent user {owner_id}")
                issues_found += 1
        ...
        for installed_id in game_data.installed:
            if installed_id not in data_store.users:
                print(f"Error: Game {release_key} installed by non-existent user {installed_id}")
                issues_found += 1
            elif installed_id not in game_data.owners:
                print(f"Error: Game {release_key} installed by user {installed_id} who doesn't own it")
                issues_found += 1
