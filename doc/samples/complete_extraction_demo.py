...
def get_user_info(conn):
    ...
    user = users[0]
    return {"user_id": user[0], "username": user[1] if len(user) > 1 else "Unknown"}
...
def generate_statistics(game_list, user_info):
    ...
    for game in game_list.values():
        igdb_platform = (
            game["igdb_key"].split("_")[0] if "_" in game["igdb_key"] else "unknown"
        )
    ...
    return {
        ...
        "installation_rate": (
            f"{installed_count/total_games*100:.1f}%" if total_games > 0 else "0%"
        ),
